...
| 公网页面 | `https://hongkongxiao.ilelezhan.cn/hk2/` |
| WSS 路径 | `wss://hongkongxiao.ilelezhan.cn/smartlife-primary-hk2-ws` |
| systemd 前缀 | `smartlife-primary-hk2-*` |

三个端口只监听回环地址，不直接暴露公网。Nginx 只新增两个精确 location，不替换现有 `/` 首页和 `/smartlife-primary-ws`。变更前备份站点配置，变更后先运行 `nginx -t`，只执行平滑 reload，不 restart。
...
- `deploy/nginx/hk2.locations.conf`：仅包含本项目两个 location，部署时插入现有 HTTPS server block
...
3. `https://hongkongxiao.ilelezhan.cn/hk2/` 返回本项目页面和最新静态资源。
