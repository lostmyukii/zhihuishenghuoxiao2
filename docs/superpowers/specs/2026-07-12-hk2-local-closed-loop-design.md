...
| 水滴传感器 | `D8 / GPIO8` | 实板标定为 `INPUT_PULLUP`、低电平触发；高电平为干燥 |
| 火焰传感器 DO/SIG | `GPIO11` | 实板标定为 `INPUT_PULLDOWN`、高电平触发；AO 不接 |
...
### 6.1 CH340 Web Serial 启动

实板通过 CH340 UART0 枚举。Dashboard 打开 Web Serial 后必须主动将 `dataTerminalReady=false`，短暂设置 `requestToSend=true` 约 `120ms` 产生硬复位，再将 `requestToSend=false` 并等待约 `450ms` 启动。这样避免浏览器打开串口后把 ESP32-S3 保持在复位状态。

连接后 `3.5s` 内没有新鲜 `hello/telemetry` 时，页面保持离线并提示断开后重新连接；不得把“串口已授权”显示成“开发板在线”。
