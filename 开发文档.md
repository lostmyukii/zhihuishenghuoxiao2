...
本项目面向小学组智慧生活赛项，核心作品为 **N16R8 智慧安心节能小屋**。作品用 N16R8 ESP32-S3 采集家庭小屋中的光照、温湿度、声音、人体与安全状态，自动控制灯、风扇、窗帘、RGB 和蜂鸣器，并通过 MQTT 与网页看板可视化展示真实状态。
...
| 智能控制 | 光照/温度/安全阈值触发灯、风扇、RGB、蜂鸣器 | 遮光开灯，升温开风扇，异常报警 |
| 语音交互 | 网页语音或小智语音转安全白名单命令 | “开始学习”“我要出门”“家里安全吗”触发真实状态 |
| 节能响应 | 无人或光照充足时关闭灯/风扇，显示节能分 | 节能分上升，网页显示节能原因 |
...
| `assets/n16r8-hk2-wiring-diagram.png` | image 模型生成的硬件接线示意图 |
...
图是 image 模型生成的施工辅助图，精确接线以本节表格为准。真实套装接线时，N16R8 ESP32-S3 主板在下方，S3 传感器扩展板叠在上方，模块线插扩展板上表面的白色 `G-V-S` 接口。
...
| 学习区 | DHT11/DHT22 | `D14 / GPIO14` | 温湿度采集、风扇阈值 | 是 |
| 学习区 | 学习灯/继电器 | `GPIO48` | 低压灯或继电器指示灯 | 是 |
| 学习区 | 风扇 PWM | `D11 / GPIO11` | 温度高通风、安全排风 | 是 |
| 学习区 | 舵机窗帘 | `D9 / GPIO9` | 休息/节能窗帘模型 | 建议 |
| 厨卫安全区 | MQ-2 AO | `ADC2 / GPIO2` | 烟雾/燃气风险 | 建议 |
| 厨卫安全区 | 水滴/漏水 | `D8 / GPIO8` | 漏水提醒 | 可选 |
| 厨卫安全区 | 火焰模拟 | `D6 / GPIO6` | 火源异常模拟 | 可选 |
| 玄关安防区 | PIR 人体红外 | `D5 / GPIO5` | 有人/无人、离家入侵 | 是 |
| 玄关安防区 | RGB 灯环 | `GPIO47` | 状态灯、报警红灯 | 建议 |
| 玄关安防区 | 蜂鸣器 | `D13 / GPIO13` | 短提醒、报警 | 建议 |
| 中控区 | OLED | `SDA=GPIO41, SCL=GPIO42` | 本地数据显示 | 是 |
| 中控区 | 8 键 AD | 候选 `GPIO3` 或 `GPIO10` | 模式切换、阈值调整 | 需先标定 |
...
4. 接核心执行器：学习灯/继电器 `GPIO48`、风扇 PWM `GPIO11`。
5. 接安全扩展：MQ-2 `GPIO2`、水滴 `GPIO8`、火焰模拟 `GPIO6`、蜂鸣器 `GPIO13`、RGB `GPIO47`。
6. 接舵机窗帘 `GPIO9`，如风扇需要方向控制，再接 `GPIO12`。
7. 单独测试 8 键 AD，确认本板是 `GPIO3` 还是 `GPIO10` 后再写入固件。
8. 最后接 USB-C 到电脑，打开串口或 Web Serial。
...
| 模式 | 触发方式 | 自动规则 | 执行反馈 |
| --- | --- | --- | --- |
| `home` 居家 | 默认、网页、语音“回到居家” | 常规采集，安全异常优先 | OLED HOME，RGB 柔和色 |
| `study` 学习 | 网页、语音“开始学习”、8 键 A | 光暗开灯，温高开风扇，声音高短提醒 | 灯、风扇、OLED、网页原因 |
| `away` 离家 | 网页、语音“我要出门”、8 键 C | 关闭灯/风扇，PIR 有人触发入侵 | RGB 红、蜂鸣器、网页告警 |
| `energy` 节能 | 网页、语音“开启节能”、8 键 D | 光照充足或无人时关灯/风扇 | 节能分、节能原因、RGB 绿 |
...
安全异常应覆盖普通手动命令。例如手动把风扇关掉后，MQ-2 超阈值仍应把风扇拉到报警风速，并把 RGB 置红。
...
```json
{"type":"telemetry","mode":"study","sensors":{"light":32,"sound":18,"temperature":28.4,"humidity":56,"pir":true,"mq2":21,"water":false,"flame":false},"actuators":{"lamp":true,"fan":60,"curtain":45,"rgb":"blue","buzzer":false},"alerts":[],"energy":{"score":82,"reason":"光线不足开启学习灯，温度偏高开启风扇"},"health":{"profileId":"smartlife-primary-safe-energy-home-v1","thresholdFocus":"lightThreshold","buzzer":"enabled"}}
```
...
- 所有网页按钮、语音意图、8 键 AD 和 MQTT command 都进入同一套命令处理。
...
插板电脑的 Chrome/Edge 页面承担 USB 网关角色，页面必须保持打开。Safari/iPhone 可作为观看端，不作为 USB 直连端。
...
- 实现光暗开灯、温高开风扇、节能关灯/关风扇。
- 命令处理返回 `ack`。

### 阶段 3：安全与输入扩展

- 加 MQ-2、水滴、火焰模拟、蜂鸣器、RGB。
- 加安全优先级规则。
- 实板标定 8 键 AD 的 GPIO 与 ADC 区间。
