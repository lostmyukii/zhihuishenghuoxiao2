...
| 厨卫安全区 | 水滴/漏水 | `D8 / GPIO8` | 实板标定为低电平触发；高电平为干燥 | 是 |
| 厨卫安全区 | 火焰模拟 | `GPIO11` 信号口 | 实板标定为 `DO/SIG` 高电平触发 | 是 |
...
- MQ-2 上电前 `60s` 显示 `warming`，不参与报警；当前实板空闲基线约 `61%`，默认安全阈值已标定为 `70%`，仍可在 Dashboard 调整。
- 所有外部电源和模块必须与 N16R8 共地。
- 水滴传感器 `DO/SIG -> GPIO8`，实板标定为低电平触发；干燥时应为高电平。
- 火焰传感器只接 `VCC/GND/DO`，其中 `DO/SIG -> GPIO11`，实板标定为高电平触发；`AO` 不接，不使用明火。
...
```json
{"type":"telemetry","ts":62000,"mode":"study","sensors":{"light":32,"lightRaw":1310,"sound":18,"soundRaw":738,"temperature":28.4,"humidity":56.0,"pir":true,"mq2":61,"mq2Raw":2498,"water":false,"flame":false},"actuators":{"lamp":true,"buzzer":false},"alerts":[],"thresholds":{"light":35,"temperature":29.0,"sound":70,"mq2":70},"energy":{"score":88,"reason":"study-mode-comfort"},"display":{"lines":["HK2 SAFE HOME"]},"health":{"profileId":"smartlife-primary-safe-energy-home-v1","dht":"ok","mq2":"ready","oled":"ready","buzzer":"enabled","relaySafety":"lowVoltageOnly","uptimeMs":62000}}
```
...
插板电脑的 Chrome/Edge 页面承担 USB 网关角色，页面必须保持打开。实板闭环已验证成功，当前进入 HTTPS + WSS + MQTT 云端阶段。Safari/iPhone 可作为云端观看端，但不作为 USB 直连端。开发板在线仍只由新鲜 `hello` 或 `telemetry` 判定，WSS 在线不能冒充开发板在线。
...
2026-07-12 冻结的公网隔离资源：

| 服务 | 实际端口或路径 |
| --- | --- |
| 公网域名 | `https://hongkongxiao2.ilelezhan.cn/` |
| Dashboard 静态站 | `127.0.0.1:19367` |
| WSS Relay | `127.0.0.1:19366` / `/smartlife-primary-hk2-ws` |
| MQTT Broker | `127.0.0.1:19383` |

三个服务使用 `/home/ubuntu/smartlife-primary-hk2` 和 `smartlife-primary-hk2-*` systemd 前缀，不复用、不停止服务器上的初中组或作品1服务。MQTT 只监听回环地址，不对公网开放。

服务器部署与隔离验证结果见 `公网部署验证报告_20260712.md`。服务器链路已通过，最终还需在插板电脑 Chrome/Edge 中完成真实 Web Serial 双设备终验。
