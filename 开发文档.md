...
4. 接核心执行器：学习灯/继电器 `GPIO48`、蜂鸣器 `GPIO13`。
5. 接安全扩展：MQ-2 `GPIO2`、水滴 `GPIO8`、火焰模拟 `GPIO6`。
6. 单独测试 8 键 AD，确认本板是 `GPIO3` 还是 `GPIO10` 后再写入固件。
7. 最后接 USB-C 到电脑，打开串口或 Web Serial。
...
| `study` 学习 | 网页、语音“开始学习”、8 键 A | 光暗开灯，温度或声音超阈值短提醒 | 学习灯、蜂鸣器、OLED、网页原因 |
| `away` 离家 | 网页、语音“我要出门”、8 键 C | 关闭学习灯，PIR 有人触发入侵 | 蜂鸣器、OLED、网页告警 |
| `energy` 节能 | 网页、语音“开启节能”、8 键 D | 仅在有人且光线暗时允许开灯 | 学习灯、节能分、节能原因、OLED 与网页 |
...
```json
{"type":"telemetry","mode":"study","sensors":{"light":32,"sound":18,"temperature":28.4,"humidity":56,"pir":true,"mq2":21,"water":false,"flame":false},"actuators":{"lamp":true,"buzzer":false},"alerts":[],"energy":{"score":88,"reason":"光线不足，已开启学习灯"},"health":{"profileId":"smartlife-primary-safe-energy-home-v1","thresholdFocus":"lightThreshold","buzzer":"enabled"}}
```
